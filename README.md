# Pelorus

A ground-up, memory-safe web browser for Windows and Linux.

> **Working name.** A pelorus is a sighting compass with no magnet — it reads
> bearings relative to your own vessel rather than to any external reference.
> Provisional; the codebase is built so it can be replaced in a day.
> See `docs/adr/000-name.md` (Phase 0 deliverable).

## The memory-safety claim

Memory-safe application code, over a small audited unsafe core, with process
isolation for the parts that cannot be made safe.

Not "written in Rust, therefore safe". The dependency closure contains real
unsafe — stylo, boa_gc, wgpu and the C GPU drivers beneath it, swash. Safety
ends at those boundaries, which is why the sandbox is the primary control and
Rust is what reduces how often it has to save you. See `docs/build-spec.md` §4.

## Status

**Phase 2 — sandbox skeleton.** Phases 0 and 1 are merged; Phase 2's gate
passes on Windows and Linux. A content process launches under a sandbox policy
on both platforms, or does not launch at all. There is no engine yet: the DOM
is Phase 4 and nothing renders a page.

Nothing is distributed to anyone before Phase 20 completes (invariant 10) — the
update channel and signed release pipeline must exist before a build leaves
this machine.

Per-phase gate reports are in `docs/`.

## Size

<!-- SIZE:BEGIN -->

| | Files | Code | Comment | Blank |
|---|---:|---:|---:|---:|
| Rust — crate sources | 25 | 2,629 | 1,381 | 398 |
| Rust — tests | 3 | 281 | 134 | 63 |
| Rust — fuzz targets | 4 | 125 | 55 | 16 |
| CI scripts | 16 | 879 | 367 | 180 |
| **Total** | **48** | **3,914** | **1,937** | **657** |

Plus 4,357 lines of specification and decision records across 21 documents.

Generated by `ci/count-lines.sh`, last run against the tree at `648a836`. Regenerate rather than edit — a number nobody measured is the kind this project keeps getting wrong. `ci/count-lines.sh --check` reports whether this is stale.

<!-- SIZE:END -->

## Where things are

| | |
|---|---|
| `CLAUDE.md` | Working agreement. Loaded every session. |
| `docs/build-spec.md` | Full specification. Read §9 for the current phase. |
| `docs/adr/` | Architecture decision records. |
| `docs/backlog.md` | Out-of-phase defects. |
| `ci/` | Gate scripts. Runnable locally; CI invokes these rather than reimplementing them. |

## License

Undecided — open decision 9 in `docs/build-spec.md` §13, along with whether
this repository is public before Phase 20.
