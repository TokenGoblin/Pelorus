...
7. **Reproducible builds.** Same source + same toolchain + same pinned build-time generators = same binary hash on both platforms. *Amended in Phase 5 by ADR 023: `stylo`'s `build.rs` generates its property definitions through Python 3 and Mako, so the interpreter is a build input. It is pinned to an exact version in CI and named in the build manifest, and `ci/gate-reproducible.sh` asserts the pin is present rather than inheriting whatever is on PATH. The claim is weaker than the one this invariant carried through Phases 0-4 and the release documentation says the weaker thing.*
...
*Gate:* builds on both OSes; hashes reproducible across two machines with different paths and usernames; supply-chain checks green; the per-crate unsafe rules hold (`forbid(unsafe_code)` everywhere except the two crates ADR 008 and ADR 024 name, each with its own stated rule); brand-leak gate proven by a deliberate violation failing CI.
